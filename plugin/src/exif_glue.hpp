...
// （タグの更新・GPS の付与・メタデータ除去）は扱わない。
//
// 読み取りエンジンは easyexif（BSD-2-Clause / 依存なし）。以前は libexif +
// libiptcdata を使う実装だったが、どちらも LGPL のコピーレフトで、静的リンクすると
// 再配布する側にライブラリ差し替えの手段を用意する義務が生じるため、パーミッシブな
// easyexif に置き換えた。書き込み機能を足そうとすると同じ罠に戻るので足さないこと。
#pragma once
