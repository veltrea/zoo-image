...
これはライセンス上の意図的な判断です。画質を落とさずメタデータだけ書き換えるには
libexif / libiptcdata が必要ですが、どちらも LGPL（コピーレフト）で、これを取り込むと
このプラグインを再配布する人（顧客のソリューションに組み込んで納品する開発者を含む）に、
ライブラリを差し替えて再リンクできる手段を用意する義務が生じます。ZooImage は読み取りに
easyexif（BSD-2-Clause）を使うことで、義務を帰属表示のみに保っています。
